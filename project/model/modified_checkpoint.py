...
import fsspec      # library for handling various filesystems
import fsspec.utils
import torch
from torch.cuda.amp import GradScaler
from torch.optim.lr_scheduler import _LRScheduler

import physicsnemo # framework for physics-informed machine learning
# aliases:
from physicsnemo.distributed import DistributedManager # helper class for ditributed training
from physicsnemo.launch.logging import PythonLogger    # loggering tool to print messages
...
# function to return the correct file path for a checkpoint
...
# take a list of models and return a dictionary with unique model names as keys
...
def save_checkpoint(
    ...
    output_filename = _get_checkpoint_filename(
        path, base_name, index=epoch, saving=True, model_type="pt"
    )
    if epoch:
...
def load_checkpoint(
    ...
    if models:
        ...
        for name, model in models.items():
            ...
            # Get full file path / name
            file_name = _get_checkpoint_filename(
                path, base_name=name, index=epoch, model_type=model_type
            )
    ...
    checkpoint_filename = _get_checkpoint_filename(path, base_name=base_name, index=epoch, model_type="pt")
